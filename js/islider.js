...
(function (global) {
    ...
    iSliderPrototype._renderItem = function (el, dataIndex) {
        ...
        var insertImg = function renderItemInsertImg() {
            ...
            if (self.isOverspread) {
                el.style.cssText += 'background-image:url(' + item.content + ');background-repeat:no-repeat;background-position:50% 50%;background-size:100%';
    ...
    iSliderPrototype.endHandler = function (evt) {
        ...
        var iSliderNum = $(".iSlider-num .imgnum");
        var iSliderNumMax = $(".iSlider-num .imglen").text();
        ...
        if (offset[axis] >= boundary && absReverseOffset < absOffset) {
			if(this.slideIndex + 1 > this.data.length - 1){
				$(".iSlider-des").show();
			}else{
				iSliderNum.text(iSliderNum.text() == 1 ? 1 : parseInt(iSliderNum.text())-1);
			}
            this.slideTo(this.slideIndex - 1);
				$(".iSlider-title").css("max-height", "1.8rem");
            ...
            this.slideTo(this.slideIndex + 1);
            iSliderNum.text(iSliderNum.text() == iSliderNumMax ? iSliderNumMax : parseInt(iSliderNum.text())+1);
            $(".iSlider-title").css("max-height", "1.8rem");
            $(".iSlider-des .arrow-down").show();
        }
        else {
